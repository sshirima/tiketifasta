...
class MerchantSchedulePaymentController extends BaseController
{
    ...
    private function setMerchantReportColumns($table)
    {
        ...
        $table->addColumn('payment_ref')->setTitle('Reference')->isSearchable()->isCustomHtmlElement(function($entity, $column){
            return '<a href="'.route('admin.merchant_schedule_payments.details', $entity['id']).'">'.$entity['reference'].'</a>';
        });

        $table->addColumn('name')->setTitle('Merchant name')->setCustomTable('merchants')->isSortable()->isSearchable()->isCustomHtmlElement(function($entity, $column){
            return $entity['merchant_name'];
        });
        ...
        $table->addColumn('transfer_status')->setTitle('Transfer status')->isSortable()->isSearchable()->isCustomHtmlElement(function($entity, $column){
            return $entity['transfer_status']?'<div class="label label-success">Sent</div>'
                :'<div class="label label-warning">Pending</div>';
        });

        $table->addColumn('account_number')->setTitle('Receiver')->setCustomTable('merchant_payment_accounts')->isSortable()->isSearchable()->isCustomHtmlElement(function($entity, $column){
