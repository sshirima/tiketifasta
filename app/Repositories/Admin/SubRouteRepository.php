...
class SubRouteRepository extends BaseRepository
{
    ...
    {
        $locations = SubRoute::with('sourceLocation')->distinct()->select(['source'])->groupBy(['source'])->get();
        $array = array('Select location');
        foreach ($locations as $location) {
            $array[$location->sourceLocation->id] = $location->sourceLocation->name;
        }
        return $array;
    }

    public function destinationLocations(){
        $locations = SubRoute::with('destinationLocation')->distinct()->select(['destination'])->groupBy(['destination'])->get();
        $array = array('Select location');
        foreach ($locations as $location) {
            $array[$location->destinationLocation->id] = $location->destinationLocation->name;
        }
        return $array;
    }
