    ...
    <section class="content container-fluid">
        <div class="nav-tabs-custom">
            <div class="nav nav-tabs">
                <li class="{{Request::is('admin/stations/*/edit') ? 'active' : ''}}"><a href="{{route('admin.stations.index')}}" >@lang('Edit station')</a></li>

            </div>
            <div class="tab-content">
                <form class="form-horizontal" role="form" method="post" enctype="multipart/form-data"
                      action="{{route('admin.stations.update', $station->id)}}"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            {!! Form::text('st_name', $station->st_name, ['class' => 'form-control', 'placeholder'=>__('Enter station name')]) !!}
                        </div>
                    </div>
                    <div class="form-group">
                        {!! Form::label('st_type', __('Station type'), ['class'=>'col-sm-3 control-label', 'for'=>'st_type']) !!}
                        <div class="col-sm-5">
                            {{Form::select('st_type',$station_types,$station->st_type,['class'=>'form-control select2'/*,isset($bus)?'disabled':''*/])}}
                        </div>
                    </div>
                    <div class="form-group">
                        {!! Form::label('location_id', __('Location'), ['class'=>'col-sm-3 control-label', 'for'=>'location_id']) !!}
                        <div class="col-sm-5">
                            {{Form::select('location_id',$locations,$station->location_id,['class'=>'form-control select2','disabled'/*,isset($bus)?'disabled':''*/])}}
                        </div>
                    </div>
                    @csrf
                    <div class="form-group">
